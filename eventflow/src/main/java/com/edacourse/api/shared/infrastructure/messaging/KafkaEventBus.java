...
public class KafkaEventBus implements EventBus, DeadLetterHandler {
    ...
    public void publish(String topic, Object event, String partitionKey) {
        ...
        producer.send(record, (metadata, exception) -> {
            if (exception != null) {
                System.err.println("Error al publicar evento: " + exception.getMessage());
            } else {
                System.out.println("Evento publicado en " + topic);
            }
        });
        producer.flush();
    }
    ...
    public <T> void onDeadLetter(String topic, Class<T> eventType, EventHandler<T> handler) {
        subscribe(topic + ".dlq", eventType, handler, "dlq");
    }
