...
public class Container {
    ...
    private Object resolveInternal(Class<?> type, Set<Class<?>> resolving) {
        if (!resolving.add(type)) {
            throw new ContainerException("Dependencia circular detectada: " + type.getSimpleName());
        }

        Registration reg = registry.get(type);
        if (reg == null) {
            throw new ContainerException("No se ha registrado la clase: " + type.getSimpleName());
        }
        ...
        try {
            ...
            if (constructors.length == 0) {
                throw new ContainerException("La clase " + reg.impl().getSimpleName() + " no tiene constructor");
            }
